...
var getPage =  function (req, res) {
    ...
    var docs = {title: 'Achievements', 
                cloudinary: JSON.stringify(cloudinary.uploader.direct_upload())};
    
    var achievementsPromise = Achievement.find({}).lean().exec();
    var teamsPromise = TeamProfile.find({}, '_id name league').lean().exec();

    var promises = [achievementsPromise, teamsPromise];

    Promise.all(promises).then(values => {
        docs.achievements = JSON.stringify(values[0]);
        docs.teams = JSON.stringify(values[1]);
        return res.render('achievements', docs);
    }).catch((err) => {
        console.log(err);
    });
};
...
var get =  function (req, res) { 
    ...
    Achievement.find(query).lean().exec().then(function (itemsFromDb) {
...
var remove = function (req, res) {
    Achievement.remove({_id: req.body.id}, function(err){
        if(err){
            return res.sendStatus(500);
        }
...
var update = function (req, res) {
    ...
    Achievement.update({_id: req.body._id}, updates, function(err){
        if(err){
            console.log(err);
            return res.sendStatus(500);
        }
