...
var getPage =  function (req, res) {
    ...
    var docs = {title: 'Trivias', 
                cloudinary: JSON.stringify(cloudinary.uploader.direct_upload())};
    
    var triviasPromise = Trivia.find({}).lean().exec();
    var teamsPromise = TeamProfile.find({}, '_id name league').lean().exec();

    var promises = [triviasPromise, teamsPromise];

    Promise.all(promises).then(values => {
        docs.trivias = JSON.stringify(values[0]);
        docs.teams = JSON.stringify(values[1]);
        return res.render('trivias', docs);
    }).catch((err) => {
        console.log(err);
    });
};
...
var get =  function (req, res) { 
    ...
    Trivia.find(query).lean().exec().then(function (itemsFromDb) {
...
var remove = function (req, res) {
    Trivia.remove({_id: req.body.id}, function(err){
        if(err){
            return res.sendStatus(500);
        }
...
var update = function (req, res) {
    ...
    Trivia.update({_id: req.body._id}, updates, function(err){
        if(err){
            console.log(err);
            return res.sendStatus(500);
        }
